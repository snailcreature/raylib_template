...
fn main() {
    ...
    while !rl.window_should_close() {
        ...
        if frame_count > 1.0 {
            value = rl.get_random_value(Range { start: -100, end: 100 });
            frame_count -= 1.0;
        }

        if rl.is_key_down(KEY_RIGHT) { ball.position.x += ball.speed * dt }
        if rl.is_key_down(KEY_LEFT) { ball.position.x -= ball.speed * dt }
        if rl.is_key_down(KEY_DOWN) { ball.position.y += ball.speed * dt }
        if rl.is_key_down(KEY_UP) { ball.position.y -= ball.speed * dt }
